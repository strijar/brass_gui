...
template <class Type> class Cmpx {
public:
    Type Re, Im;

public:
    template <class Type2> const Cmpx<Type> &operator=(const Cmpx<Type2> &New) {
        Re = New.Re;
        Im = New.Im;
        return (*this);
    }

    template <class Type2> const Cmpx<Type> &operator=(const Type2 &New) {
        Re = New;
        Im = 0;
        return (*this);
    }

    template <class Type2> void Set(Type2 NewRe, Type2 NewIm = 0) {
        Re = NewRe;
        Im = NewIm;
    }

    template <class Type2> void Set(Cmpx<Type2> New) {
        Re = New.Re;
        Im = New.Im;
    }

    void SetPhase(double Phase, double Mag = 1.0) {
        Re = Mag * cos(Phase);
        Im = Mag * sin(Phase);
    }
    ...
    // void operator /= (const Type2 Mult)
    // { Re*=Mult; Im*=Mult; }
    template <class Type2> double VectDotProd(Cmpx<Type2> X) {
...
template <class typeA, class typeB> inline double ScalProd(Cmpx<typeA> &A, Cmpx<typeB> &B) {
    return A.Re * B.Re + A.Im * B.Im;
}

template <class typeA, class typeB> inline double ScalProd(typeA Ia, typeA Qa, Cmpx<typeB> &B) {
...
template <class Type> void CmpxSqrt(Cmpx<Type> &X) {
    Type Mag   = X.Mag();
    int  NegIm = (X.Im < 0);
...
template <class Type> void CmpxSquare(Cmpx<Type> &X) {
