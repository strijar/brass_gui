...
template <class type> inline int ReallocArray(type **Array, size_t Size) {
...
template <class type> int ReallocArraySafe(type **Array, size_t OldSize, size_t NewSize) {
    type  *New;
    size_t Size;
    New = (type *) malloc(NewSize * sizeof(type));
    if (New == 0) {
        return -1;
    }
    Size = OldSize;
    ...
    if (Size) {
        memcpy(New, *Array, Size * sizeof(type));
    }
    free(*Array);
    (*Array) = New;
    return 0;
...
template <class type> inline int AllocArray(type **Array, size_t Size) {
    (*Array) = (type *) malloc(Size * sizeof(type));
    return (*Array) == 0;
}

template <class type> inline void ClearArray(type *Array, size_t Size) {
    memset(Array, 0, Size * sizeof(type));
}

template <class type> void PresetArray(type *Array, size_t Size, const type &Val) {
    size_t i;
    for (i = 0; i < Size; i++) {
        Array[i] = Val;
    }
...
template <class type> inline void CopyArray(type *Dst, type *Src, size_t Size) {
    memcpy(Dst, Src, Size * sizeof(type));
}

template <class type> inline void MoveArray(type *Dst, type *Src, size_t Size) {
    memmove(Dst, Src, Size * sizeof(type));
}

template <class type>
void PrintArray(type *Array, size_t Size, char *ValueFormat, char *IndexFormat = "%3d", size_t Columns = 10) {
    size_t Idx, Col;
...
public:
    Seq();
    ~Seq();                            // default constructor/destructor
    ...
    int  EnsureSpace(void) {
        return EnsureSpace(Len);
    }

    int SetLen(size_t NewLen) {
        Len = NewLen;
        return EnsureSpace(Len);
    }
    ...
    void Clear(void) {
        Len = 0;
    } // clear the sequence (set length to zero)

    size_t Index(type *Item) {
        return (size_t) (Item - Elem);
    }
    ...
    const Seq<type> &operator=(const Seq<type> &SrcSeq); // assignment operator
    ...
    int  Join(Seq<type> &Seq) {
        return Join(Seq.Elem, Seq.Len);
    } // join a copy of the other sequence
...
template <class type> void Seq<type>::Init() {
    Elem = 0;
    Len = Space = 0;
}
...
// CINT doesn't accept NULL pointers to realloc()
    ...
    if (Elem) {
        ...
        Elem = (type *) malloc(ReqSpace * sizeof(type));
    }
#else
    Elem = (type *) realloc(Elem, ReqSpace * sizeof(type));
#endif
...
template <class type> const Seq<type> &Seq<type>::operator=(const Seq<type> &SrcSeq) {
...
template <class type> Seq2d<type>::Seq2d() {
    Elem      = 0;
    AllocRows = Rows = AllocCols = Cols = 0;
}
...
template <class type> int Seq2d<type>::EnsureSpace(size_t rows, size_t cols) {
    ...
    if (rows > AllocRows) {
        ...
        if (Elem) {
            memcpy(New, Elem, AllocRows * sizeof(type **));
            free(Elem);
        }
        Elem = New;
...
template <class Type> class SampleBuffer {
public:
    SampleBuffer() {
        Sample    = 0;
    ...
    int EnsureSize(size_t MaxSize) { // printf(" SampleBuffer::EnsureSize(%d)\n",MaxSize);
        ...
        if (Sample) {
            ...
            Sample = (Type *) malloc(MaxSize * sizeof(Type));
        }
#else
        Sample = (Type *) realloc(Sample, MaxSize * sizeof(Type));
#endif
