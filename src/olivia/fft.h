...
public: // size must a power of 2: 2,4,8,16,32,64,128,256,...
    ...
    int Preset(int MaxSize) {
        ...
        if (ReallocArray(&BitRevIdx, Size) < 0) {
            goto Error;
        }
        if (ReallocArray(&Twiddle, Size) < 0) {
            goto Error;
        }
    ...
    template <class DataType> void Scramble(DataType x[]) {
        size_t   idx, ridx;
        DataType tmp;
        for (idx = 0; idx < Size; idx++) {
    ...
    template <class DataType> void Scramble(DataType x[], size_t ShrinkShift) {
        size_t   idx, ridx;
        DataType tmp;
    ...
    template <class BuffType, class DataType> void SeparTwoReals(BuffType Buff[], DataType Out0[], DataType Out1[]) {
    ...
    template <class BuffType, class DataType> void JoinTwoReals(DataType Inp0[], DataType Inp1[], BuffType Buff[]) {
        int idx, HalfSize = Size / 2;
        //  for(idx=0; idx<HalfSize; idx++)
        //    printf("%2d  %9.5f %9.5f  %9.5f %9.5f\n",
    ...
    template <class BuffType> void CoreProc(BuffType x[]) {
    ...
    template <class BuffType> void CoreProc(BuffType x[], size_t ShrinkShift) {
        size_t Groups, GroupSize2, Group, Bf, TwidIdx, TwidIncr;
        size_t Len   = Size >> ShrinkShift;
        size_t Size2 = Size / 2;
        size_t Len2  = Len / 2;
        for (Bf = 0; Bf < Len; Bf += 2) {
            FFT2(x[Bf], x[Bf + 1]); // first pass
        }
        // for(Bf=0; Bf<Len; Bf+=4) FFT4(x[Bf],x[Bf+1],x[Bf+2],x[Bf+3]); // second
        for (Groups = Len2 / 2, TwidIncr = Size2 / 2, GroupSize2 = 2; Groups;
    ...
    template <class BuffType> int Process(BuffType x[]) {
        Scramble(x);
        CoreProc(x);
        return 0;
    }
    ...
    template <class BuffType> int Process(BuffType x[], size_t Len) {
        if (Len < 4) {
            return -1;
        }
        if (Len == Size) {
        ...
        Scramble(x, ShrinkShift);
        CoreProc(x, ShrinkShift);
        return 0;
    }
...
private:
    ...
    template <class BuffType> inline void FFTbf(BuffType &x0, BuffType &x1, Type &W) {
        Type x1W;
    ...
    template <class BuffType> inline void FFT2(BuffType &x0, BuffType &x1) {
        Type x1W;
        x1W.Re = x1.Re;
        x1W.Im = x1.Im;
        x1.Re  = x0.Re - x1.Re;
    ...
    template <class BuffType> inline void FFT4(BuffType &x0, BuffType &x1, BuffType &x2, BuffType &x3) {
        Type x1W;
        x1W.Re = x2.Re;
        x1W.Im = x2.Im;
        x2.Re  = x0.Re - x1W.Re;
