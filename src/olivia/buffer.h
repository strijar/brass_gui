...
public:
    ...
    template <class PhaseType> void WrapPhase(PhaseType &Phase) {
        if (Phase < 0) {
            Phase += Len;
        } else if (Phase >= Len) {
            Phase -= Len;
    ...
    template <class PhaseType> void WrapDiffPhase(PhaseType &Phase) {
        if (Phase < (-(PhaseType) Len / 2)) {
            Phase += Len;
        } else if (Phase >= ((PhaseType) Len / 2)) {
            Phase -= Len;
        }
    }

    // synchronize current pointer with another circular buffer
    template <class SrcType> void operator|=(CircularBuffer<SrcType> Buffer) {
        Ptr = Buffer.Ptr;
    }
